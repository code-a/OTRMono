...
namespace Mono.OTR.Managers
{
	public class DataExchangeKeysManager
	{
		...
		public DataExchangeKeys ComputeKeys(DHKeyPair my_key_pair, BigInteger buddy_public_key)
        {
            ...
            if (buddy_public_key.CompareTo(BigInteger.One) < 0)
            ...
            if (my_key_pair.GetPublicKey().CompareTo(buddy_public_key) > 0)
            {
