...
namespace Mono.OTR.Managers
{
	...
	public class OTRSessionObjects
	{
        ...
        BigInteger _my_buddy_recent_dh_public_key = null;
        BigInteger _my_buddy_old_dh_public_key = null;
	    ...
	    public OTRSessionObjects()
        {
			...
			_my_buddy_recent_dh_public_key = BigInteger.Zero;
            _my_buddy_old_dh_public_key = BigInteger.Zero;
		...
		public bool IsSetMyBuddyFirstPublicKey(byte[] aes_key, byte[] encrypted_buddy_recent_public_key_mpi_bytes, byte[] hashed_public_key_mpi_bytes)
        {
            ...
            BigInteger _key_holder = BigInteger.Zero;
		...
		public DataExchangeKeys GetDataExchangeKeys(UInt32 my_acked_key_id, UInt32 buddy_key_id, byte[] next_buddy_key_mpi_bytes, bool re_key_my_dh_keys)
        {

            if (next_buddy_key_mpi_bytes == null || next_buddy_key_mpi_bytes.Length < 1)
                return null;


            BigInteger _key_holder = BigInteger.Zero;

            Utility.DecodeMpiFromBytes(next_buddy_key_mpi_bytes, 0, ref _key_holder);
        ...
        private DataExchangeKeys GetDataExhangeKeys (UInt32 my_key_id, UInt32 buddy_key_id)
		{
			...
			BigInteger _buddy_public_key_holder = BigInteger.Zero;
            ...
            if (_my_dh_key_pair_holder == null || _buddy_public_key_holder == BigInteger.Zero)
                return null;
        ...
        private void ReKeyBuddyKeys (BigInteger buddy_public_key)
		{

			/*is it the same public key we know?   */
			if (buddy_public_key.Equals (_my_buddy_recent_dh_public_key) || buddy_public_key.Equals (_my_buddy_old_dh_public_key)) 			
				return;
		...
		public UInt32 GetBuddyOldKeyIDInt ()
		{

			return _my_buddy_old_dh_public_key_id;
	...
	public class OTRFragmentObject
    {
